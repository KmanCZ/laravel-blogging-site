...
<a href="{{route('posts.show', ['post'=>$post->slug, "user" => $post->user])}}">
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-2">
        ...
        <div class="p-6 bg-white border-b border-gray-200">
            ...
            <p class="px-6">by<a class="hover:underline" href="{{route("users.show", ["user" => $post->user])}}"><img class="w-7 h-7 mr-1 ml-2 rounded-full inline-block" src="{{Storage::url($post->user->profile_picture)}}" alt="profile picture">{{$post->user->name}}</a></p>
