<x-app-layout>
    <x-slot name="header">
        ...
        <p>
            by<a class="hover:underline" href="{{route("users.show", ["user" => $post->user])}}"><img class="w-7 h-auto mr-1 ml-2 rounded-full inline-block" src="{{asset("storage/". $post->user->profile_picture)}}" alt="profile picture">{{$post->user->name}}</a>
        </p>
